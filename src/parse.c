...
char* search_page(const char* filename, struct book_t** book_array,
   ...
   static char* pattern[] = {
      "<table width=100\% cellspacing=1 cellpadding=1 rules=rows class=c align=center><tr",
   ...
   if(fseek(page_file, FILE_BEG, SEEK_SET) &&
      ferror(page_file)) {
        exit_and_report();
   }
      ...
      if((match = strstr(buffer_cursor, pattern[i]))) {
         if(i == BOOK_DOWNLOAD_LINK) {
            ...
            *download_url = (char*) malloc((close_tag - match + 1) * sizeof(char));
            ...
            close_tag = strchr(match, '<');
            *close_tag = '\0';
            selected_book->series = (char*) malloc((close_tag - match + 1) *
                                                    sizeof(char));
            ...
            close_tag = strchr(match, '<');
            *close_tag = '\0';
            selected_book->isbn = (char*) malloc((close_tag - match + 1) *
                                                    sizeof(char));
