...
char *download_book(struct book_t *selected_book, char **log_msg) {
  ...
  char *hostname, *path;
  int mkdir_status = mkdir(save_dir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
void generate_ref(const struct book_t *selected_book, const long book_fn_len) {
  if (*save_ref_dir) {
    long ref_dir_len;
    int mkdir_status =
        mkdir(save_ref_dir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    if (mkdir_status == ENOTDIR || mkdir_status == ENAMETOOLONG) {
    ...
    char *ref_filename =
        ecalloc((book_path_len - book_fn_len + 4) + ref_dir_len, sizeof(char));
